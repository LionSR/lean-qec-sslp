...
/-!
# BD16v2 certified with the kernel as the only trusted base
...
def ampZ : SS.BitString 7 → SS.ZSqrt235i := fun x =>
  ...
  else if x = bs false false true  true  true  true  false then AmpZ.r3 (-1)
  else if x = bs false true  false false true  false true then AmpZ.ir2 (1)
  else if x = bs false true  true  false false false true then AmpZ.ir2 (-1)
  else if x = bs true  false false false false true  true then AmpZ.ir2 (-1)
  else if x = bs true  false false true  false false true then AmpZ.ir2 (1)
  else if x = bs true  true  false true  true  true  false then AmpZ.im (1)
  else if x = bs true  true  true  false true  true  false then AmpZ.im (1)
...
def probZ : SS.BitString 7 → Int := fun x =>
  ...
  else if x = bs false false true  true  true  true  false then (3)
  else if x = bs false true  false false true  false true then (2)
  else if x = bs false true  true  false false false true then (2)
  else if x = bs true  false false false false true  true then (2)
  else if x = bs true  false false true  false false true then (2)
  else if x = bs true  true  false true  true  true  false then (1)
  else if x = bs true  true  true  false true  true  false then (1)
...
lemma amp_eq : BD16v2.code.amp = fun s => scaleD (ampZ s) := by
  funext s
  show BD16v2.code.amp s = scaleD (ampZ s)
  unfold BD16v2.code ampZ
  simp only [apply_ite scaleD, sc_im_1, sc_ir2_1, sc_ir2_neg1, sc_r3_neg1, sc_rat_1, sc_zero]
...
lemma prob_eq : BD16v2.code.prob = fun s => probQ (probZ s) := by
  funext s
  show BD16v2.code.prob s = probQ (probZ s)
  unfold BD16v2.code probZ
  simp only [apply_ite probQ, pq_1, pq_3, pq_2, pq_zero]

/-! ## Integer facts, decided by the kernel -/

theorem probZ_off : ∀ x, x ∉ BD16v2.code.supp → probZ x = 0 := by decide
theorem probZ_nonneg : ∀ x ∈ BD16v2.code.supp, 0 ≤ probZ x := by decide
theorem probZ_sum : (∑ x ∈ BD16v2.code.supp, probZ x) = 16 := by decide
theorem zbal_int : ∀ i : Fin 7,
    (∑ x ∈ BD16v2.code.supp, probZ x * (if x i = true then -1 else 1)) = 0 := by decide
theorem probamp_int : ∀ s ∈ BD16v2.code.supp,
    SS.ZSqrt235i.mul (SS.ZSqrt235i.conj (ampZ s)) (ampZ s) = AmpZ.rat (probZ s) := by
  decide
theorem ampZ_off : ∀ s, s ∉ BD16v2.code.supp → ampZ s = SS.ZSqrt235i.zero := by decide
theorem allKLZ : SS.allKLSatisfiedZ BD16v2.code.supp ampZ 2 = true := by decide


/-! ## The rational statements -/

/-- **Layer 1, certified without `native_decide`.** -/
theorem layer1_kernel : BD16v2.code.layer1OK := by
  simp only [BD16Code.layer1OK, Distance3Data.Layer1OK, BD16Code.toD3Data, prob_eq]
  refine ⟨by decide, ?_, ⟨?_, ?_, ?_⟩, ?_⟩
  · simp [BDAngleCondition, BD16v2.code]; decide
  ...
  · simp only [probQ, ← Finset.sum_div]
    rw [show (∑ x ∈ BD16v2.code.supp, ((probZ x : ℚ))) = ((16 : Int) : ℚ) by
      rw [← probZ_sum]; push_cast; ring]
    norm_num
  · intro i
    simp only [zExpectation, zSign, probQ]
    have h : ∀ x ∈ BD16v2.code.supp, ((probZ x : ℚ)) / 16 * (((if x i = true then (-1 : Int) else 1) : Int) : ℚ)
        = ((((probZ x * (if x i = true then (-1 : Int) else 1)) : Int) : ℚ)) / 16 := by
      intro x _; push_cast; ring
    rw [Finset.sum_congr rfl h, ← Finset.sum_div]
    rw [show (∑ x ∈ BD16v2.code.supp, ((((probZ x * (if x i = true then (-1 : Int) else 1)) : Int) : ℚ)))
        = ((((∑ x ∈ BD16v2.code.supp, probZ x * (if x i = true then (-1 : Int) else 1)) : Int) : ℚ)) by
      push_cast; ring, zbal_int i]
    norm_num

/-- **Probability-amplitude consistency, certified without `native_decide`.** -/
theorem probamp_kernel : BD16v2.code.probAmpConsistent := by
  intro s hs
...
theorem layer2_kernel : BD16v2.code.layer2OK 2 := by
  have h := SS.allKLSatisfied_of_Z_smul BD16v2.code.supp ampZ 2 (q := 1/4) (by norm_num) allKLZ
  rw [BD16Code.layer2OK, amp_eq]
  exact h

/-- **The full distance-3 certificate.** -/
theorem isDistance3_kernel : BD16v2.code.isDistance3 :=
  ⟨probamp_kernel, layer1_kernel, layer2_kernel⟩

theorem amp_vanish_kernel :
    ∀ s, s ∉ BD16v2.code.supp → BD16v2.code.amp s = SS.QSqrt235i.zero := by
  intro s hs
  rw [amp_eq]
...
theorem qec_kernel (P : SS.PauliString 7) (hw : P.wt ≤ 2) :
    SS.innerC (SS.BD16.state0 BD16v2.code) (SS.pauliAct P (SS.BD16.state1 BD16v2.code)) = 0
      ∧ SS.innerC (SS.BD16.state1 BD16v2.code) (SS.pauliAct P (SS.BD16.state1 BD16v2.code))
          = SS.innerC (SS.BD16.state0 BD16v2.code) (SS.pauliAct P (SS.BD16.state0 BD16v2.code)) :=
  SS.BD16.detects_weight2 BD16v2.code isDistance3_kernel amp_vanish_kernel P hw
