...
\textcolor{Slate}{\small No floating point and no \code{sorry} placeholders. (All proofs in $\CC$ = \code{Complex}.)}
\end{frame}
...
\item \code{native\_decide} discharges each finite goal automatically (runtime hardware-dependent)
\item \code{lake build} can compile the generated files (one per code)
\end{enumerate}
\end{block}
...
\item Verification tasks are independent and can be parallelized across instances
\end{itemize}
\end{exampleblock}
...
\tikz\node[draw=Teal!60, fill=Teal!6, rounded corners=3pt, inner sep=4pt,
  ...
  \textbf{certificates}: a concrete target};
\end{columns}
\end{frame}
...
{\tiny
Batch verification of 14{,}116 distance-2 instances\; $\cdot$\; distance-3 automation\\
Other BD groups (BD$_{12}$--BD$_{36}$), $n{=}8$ extensions, and exceptional groups ($2I/2T/2O$).}
\end{alertblock}
\end{frame}
