...
\begin{columns}[T,onlytextwidth]
\column{0.50\textwidth}
\begin{block}{Mechanization idea (and the speed trick)}
\footnotesize
\begin{itemize}\setlength\itemsep{1pt}
\item Pitfall: letting \code{simp} unfold \code{coeff} inside \code{Finset.sum} can create huge nested \code{if}-terms (heartbeat timeouts).
\item Fix: precompute intersections like $S_0\cap(S_0\oplus z_P)$ by \code{native\_decide}, expand each KL element into a $2$--$4$ term sum, then use \code{simp only} + \code{linarith}.
\end{itemize}
\end{block}
...
\textcolor{Slate}{\scriptsize Lean: \code{Examples/D3/NoGo/BD16\_0112335\_FromKL.lean} \,(build: \code{lake build Examples.D3.NoGo.BD16\_0112335\_FromKL}).}
\end{frame}
